...
def inner_loop_dataset(mode, penalty_mode, num_steps, pgd, digits, lambdas, timestamp, dataset, dmf, dataset_len,
        batch_size, sparse_dataset, recovery_lambda_final, recovery_step_lambda_at, recovery_step_lr_at, model_num):
    folder = f"image_lambda_runs/{dataset}/{mode}/{penalty_mode.replace(' ','_')}/num-recovery-steps_{num_steps}/{pgd}/{timestamp}/{model_num}"
    for i, lambd in enumerate(lambdas):
...
def mega_loop():
    ...
    for model_num in ['0049', '0199', '0024', '0099', '0149']:
        dmf = f'train_runs/0083-May14_10-19-51_adv-train-fresh-full_cifar/ckpt/model_opt_sched/model_opt_sched_{model_num}.pt'
